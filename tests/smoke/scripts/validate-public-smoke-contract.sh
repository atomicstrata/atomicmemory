...
jq -e '
  all(.rows[];
    ...
    (.publish_status | IN("published", "implemented_publish_pending", "private_unpublished")) and
...
jq -e '
  all(.rows[] | select(.name | IN("@atomicmemory/langchain", "@atomicmemory/langgraph", "@atomicmemory/mastra"));
    .publish_status == "implemented_publish_pending" and
...
jq -e '
  all(.rows[] | select(.name | IN("@atomicmemory/codex-plugin", "@atomicmemory/cursor-plugin"));
    .publish_status == "private_unpublished" and
